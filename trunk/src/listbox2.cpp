	...
	if (o->icons) {
		...
		if (flags & EAknListBoxMultipleSelection) {
			...
			TRAP(error,
				...
				markBitmap = CEikonEnv::Static()->CreateBitmapL(KAvkonBitmapFile, EMbmAvkonQgn_indi_marked_add);
				CleanupStack::PushL(markBitmap);
				markBitmapMask = CEikonEnv::Static()->CreateBitmapL(KAvkonBitmapFile, EMbmAvkonQgn_indi_marked_add_mask);
				CleanupStack::PushL(markBitmapMask);
				o->icons->AppendL(CGulIcon::NewL(markBitmap, markBitmapMask));
				CleanupStack::Pop(2);
			);
#endif
